...
min_year = 2015 # minimum year to be included

# -------------------------------------------------------------------------
# What comes below is the same as found in 1_single/clean_data.R
# In case of any change in that code, just paste it below
...
my_data = my_data %>% dplyr::filter(year >= min_year) # from 2015
...
# Filter year:
eff_data = eff_data %>% filter(year >= min_year)
