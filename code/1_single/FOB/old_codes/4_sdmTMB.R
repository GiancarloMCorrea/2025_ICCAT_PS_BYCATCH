...
plot_folder = file.path('figures', this_type, 'sdmTMB')
dir.create(plot_folder, recursive = TRUE, showWarnings = FALSE)

# Define model folder:
model_folder = file.path('models', this_type, 'sdmTMB')
dir.create(model_folder, recursive = TRUE, showWarnings = FALSE)

# N sp to build models
n_mod_sp = 47 # model the first N species

# Longitude latitude names in data:
lonlat_cols = c('longitude', 'latitude')

# -------------------------------------------------------------------------
# Read data in:
weight_data = readRDS(file = file.path('data', this_type, 'weight_data.rds'))
load(file.path('data', this_type, 'effPoints.RData'))
...
prev_mesh = fm_mesh_2d( tmp_data[,lonlat_cols], cutoff = 2 )
...
mesh_df = data.frame(Lon = prev_mesh$loc[,1], Lat = prev_mesh$loc[,2]) %>% st_as_sf(coords = c("Lon", "Lat"), crs = 4326, remove = FALSE)
plot_dat = tmp_data %>% st_as_sf(coords = lonlat_cols, crs = 4326, remove = FALSE)
p1 = ggplot() + 
...
for(i in 1:n_mod_sp) {
  ...
  pres_data = pres_data %>% st_as_sf(coords = lonlat_cols, crs = 4326, remove = FALSE)
  p1 = ggplot(pres_data) + 
  ...
  zero_years = plot_dat$year[which(plot_dat$bycatch == 0)]
  if(length(zero_years) > 0) sp_data = sp_data %>% dplyr::filter(!(year %in% zero_years))
  ...
  sp_mesh = sdmTMB::make_mesh(data = sp_data, xy_cols = lonlat_cols, 
                              mesh = fm_mesh_2d( sp_data[,lonlat_cols], cutoff = 2 )) 
  
  xydat = sp_data %>% filter(year == 2021)
  coords = xydat[,lonlat_cols]
  w = fields:::rdist(coords)
  ape::Moran.I(x = xydat$bycatch, w = w)
  
  
  tmpmod = sdmTMB(
    data = sp_data,
    formula = list(bycatch ~ 0 + fyear + quarter + trop_catch + thetao + mlotst + nppv,
                   bycatch ~ 0 + fyear + quarter + trop_catch + thetao + mlotst + nppv),
    mesh = sp_mesh, 
    time = 'year', 
  ...
  summdf_1 = tidy(tmpmod, model = 1)
  summdf_1 = summdf_1 %>% mutate(z_score = estimate/std.error,
                                 p_value = 2*pnorm(-abs(z_score)))
  summdf_1$term[which(summdf_1$p_value >= 0.01)]
  summdf_2 = tidy(tmpmod, model = 2)
  summdf_2 = summdf_2 %>% mutate(z_score = estimate/std.error,
                                 p_value = 2*pnorm(-abs(z_score)))
  summdf_2$term[which(summdf_2$p_value >= 0.01)]
  
  # Get pvalue = 2*pnorm(zscore, mean = 0, sd = 1, lower.tail = FALSE)
  # zscore = estimate/se
  
  tmpmod2 = sdmTMB(
    data = sp_data,
    formula = list(bycatch ~ 0 + fyear + quarter + trop_catch + mlotst,
                   bycatch ~ 0 + fyear + thetao),
    mesh = sp_mesh, 
    time = 'year', 
    family = delta_lognormal(),
    spatial = list("on", "on"),
  ...
  summdf_1 = tidy(tmpmod2, model = 1)
  summdf_1 = summdf_1 %>% mutate(z_score = estimate/std.error,
                                 p_value = 2*pnorm(-abs(z_score)))
  summdf_1$term[which(summdf_1$p_value >= 0.01)]
  summdf_2 = tidy(tmpmod2, model = 2)
  summdf_2 = summdf_2 %>% mutate(z_score = estimate/std.error,
                                 p_value = 2*pnorm(-abs(z_score)))
  summdf_2$term[which(summdf_2$p_value >= 0.01)]
  ...
  tmpmod3 = sdmTMB(
    data = sp_data,
    formula = bycatch ~ 0 + fyear + quarter + trop_catch + thetao + mlotst + nppv,
    mesh = sp_mesh, 
    time = 'year', 
    family = tweedie(),
    spatial = list("on"),
  ...
  summdf = tidy(tmpmod3, model = 1)
  summdf = summdf %>% mutate(z_score = estimate/std.error,
                             p_value = 2*pnorm(-abs(z_score)))
  summdf$term[which(summdf$p_value >= 0.01)]
  
  tmpmod4 = sdmTMB(
    data = sp_data,
    formula = bycatch ~ 0 + fyear + trop_catch,
    mesh = sp_mesh, 
    time = 'year', 
    family = tweedie(),
    spatial = list("on"),
  ...
  summdf = tidy(tmpmod4, model = 1)
  summdf = summdf %>% mutate(z_score = estimate/std.error,
                             p_value = 2*pnorm(-abs(z_score)))
  summdf$term[which(summdf$p_value >= 0.01)]
  
  
  eff_plot = visreg::visreg(tmpmod4, xvar = "trop_catch")
  par(mar = c(4,4,1,1))
  plot(eff_plot)
  
  
  newpred_df = effPoints %>% mutate(fyear = as.factor(year))
  predictions = predict(tmpmod4, newdata = newpred_df, return_tmb_object = TRUE)
  PredGrid = predictions$data
  PredGrid$est_org = tmpmod4$family$linkinv(PredGrid$est)
  ...
  # Fit tinyVAST model
  mod_ln = tryCatch(sdmTMB(
                        data = sp_data,
                        formula = list(bycatch ~ 0 + fyear + thetao,
                                       bycatch ~ 0 + fyear + thetao),
                        mesh = sp_mesh, 
                        time = 'year', 
                        family = delta_lognormal(),
